...
<main>
    <div class="main">
        <div class="search">
            <form class="form-inline float-left" action="LandloardServlet?action=findByMap" method="post">
        ...
        <div class="table-box">
			...
			<c:forEach items="${landloardList }" var="u">
			                <tr>
			                  <!--   <th scope="row">1</th> -->
			                    <td>${u.realname}</td>
            ...
            <ul class="pagination">
                <li class="page-item"><a class="page-link" href="LandloardServlet?action=list&p=1">&lt;&lt;</a></li>
                             
                 <c:if test="${cp>1}">
                  <li class="page-item"><a class="page-link" href="LandloardServlet?action=list&p=${cp-1}">上一页</a></li>
                            <a  href="">上一页</a>
                        </c:if>
                        <%--        循环显示页码--%>
                        <c:forEach begin="${cp-2>1 ? (cp-2) :1}" end="${cp+2>tp?tp:(cp+2)}" var="e">
                            <%--            判断是否是当前页--%>
                            <c:if test="${cp==e}">
                            <li class="page-item"><a class="page-link" href="LandloardServlet?action=list&p=${e}">${e}</a></li>
                        ...
                        <%--        判断是否有下一页--%>
                        <c:if test="${cp<tp}">
                         <li class="page-item"><a class="page-link" href="LandloardServlet?action=list&p=${cp+1}">下一页</a></li>                        
                        </c:if>
             
                <li class="page-item"><a class="page-link" href="LandloardServlet?action=list&p=${tp}">&gt;&gt;</a></li>
            </ul>
        </div>
        </div>
